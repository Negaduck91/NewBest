...
#include "src/lib/FacadeScaff.hxx"        //Recoding 99% done
...
int main(int argc, char* argv[]){
    ...
    //the first is the length_ of the Scaffold
    //and the second value is the height_ 
    if(argc == 3 ){
        double L,H=0;
        ...
        H = std::stod(args[1]);         //...
        Scaff::FacadeScaff NewScaff(L,H);
        ...
        do{
            ...
            switch(choice){
			    case 1: 
					...
					break;
			    case 2: 
                    Scaff::PrintListOfnum_of_comps_(&std::cout,NewScaff.GetComponents(),
                                            NewScaff.GetDimensions());
                    break;
			    case 3: 
                    CS = Inputconstruction_site_Name();
					WriteCalcednum_of_comps_(NewScaff.GetComponents(), NewScaff.GetDimensions(),CS);
					Scaff::AddDBEntry(CS);	
                    break;
			    case 4: exit(0);
		    }
